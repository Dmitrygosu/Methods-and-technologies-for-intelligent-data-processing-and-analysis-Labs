# Генератор тестов на базе локальной LLM: модель test-inzhener пишет
# pytest-тесты для целевого модуля, скрипт проверяет их на валидный синтаксис
# (до 3 попыток), гоняет pytest, при падениях отправляет протокол модели
# на доработку, а в конце просит модель проанализировать итоги на русском
# и складывает всё в reports/<модуль>.md.
#
