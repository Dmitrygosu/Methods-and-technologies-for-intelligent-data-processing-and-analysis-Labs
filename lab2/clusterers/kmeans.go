...
func (km *KMeans) Train(trainSet *data.Dataset, maxIterations int) {
	if len(trainSet.Instances) == 0 {
		return
	}
	
	km.numFeatures = trainSet.NumFeatures
	km.centroids = make([][]float64, km.k)
	
	for i := range km.centroids {
		km.centroids[i] = make([]float64, km.numFeatures)
		randomIdx := rand.Intn(len(trainSet.Instances))
		copy(km.centroids[i], trainSet.Instances[randomIdx].Features)
	}
...
func (km *KMeans) PredictBatch(testSet *data.Dataset) []int {
